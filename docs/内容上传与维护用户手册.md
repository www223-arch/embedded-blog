...
- 项目图片会被移动到 `gallery` 第一位，主站卡片会优先使用它作为视觉图。
- 个人分享图片会写入 `cover` 字段。

项目图集可以在素材页下方直接维护：

- “上移 / 下移”用于调整展示顺序。
- “移除”只会从 `gallery` 字段移除，不会删除图片文件。

点“删除”后：

- 素材文件会从 `public/images` 或 `public/videos` 中删除。
- 如果它正好是项目图集或个人分享封面，Studio 会同步从字段里移除。
- 如果正文 Markdown 已经手动引用了这张图或视频，需要自己删除对应 Markdown 片段。

注意：插入正文、加入图集、设为封面、调整图集后，只是进入浏览器草稿状态。你仍然需要点击顶部“保存”，才会写回 Markdown 文件。删除素材文件本身会立即生效。
...
点击顶部“预览差异”可以在保存前查看本次修改：

- Frontmatter 会用结构化文本展示增删行。
- Markdown 正文会按行展示新增和删除。
- 可以从差异弹窗继续编辑，也可以直接保存这些修改。
...
### 2.8 使用 Markdown 快捷编辑

Content Studio 的 Markdown 编辑区上方有一排快捷按钮：
...
也可以把图片、GIF、SVG、MP4 或 WebM 直接拖到 Markdown 编辑区：

- 文件会先上传到当前内容对应的素材目录。
- 上传成功后会自动在光标位置插入 Markdown 图片语法或 `video` 富内容块。
- 素材文件上传会立即写入 `public/images` 或 `public/videos`。
- 正文引用仍然只是浏览器草稿，需要点击“保存”才会写回 Markdown 文件。

### 2.9 哪些情况仍建议直接改 Markdown
...
想把某个项目做成带轨迹与阶段场景的沉浸展示时，将 `presentation` 改为 `immersive`，填写 `updatedAt` 与 `currentFocus`，再按需要选择 `narrative`（`chronicle`、`field-notes`、`chapters`）和 `visualPreset`（`orbit`、`signal`、`archive`）。普通项目不必填写这些增强内容。

可以在正文任意位置插入下列区块；它们按出现顺序形成项目的阶段、问题和下一步：
...
`media` 只能使用 `/images/` 或 `/videos/` 下的路径，不能包含 `..` 或反斜杠。Content Studio 的 Markdown 工具栏提供“阶段”“问题”“下一步”快捷插入；专属 3D 场景属于少量项目的代码增强，不是每次发布都必须配置。
