...
backgroundImage: /images/projects/smart-car/background.jpg
...
项目详情背景是可选配置，也可以直接在 Content Studio 的项目元数据区调整：

- `backgroundImage`：项目详情页使用的图片路径。留空时继承网站默认背景。
- `backgroundPosition`：图片视觉焦点，可选 `center`、`top`、`bottom`、`left`、`right`。
- `backgroundTone`：为了让正文始终清晰而叠加的遮罩，可选 `soft`、`balanced`、`strong`。

推荐把素材放在 `public/images/projects/<项目 ID>/` 中。背景的职责是衬托项目和当前界面，不必每张都承担强烈表达；优先选择真实项目照片、设备照片或有明确授权的公共素材，并在同目录记录来源和许可。先用 `balanced`，只有图片过亮、过杂时再切到 `strong`。
