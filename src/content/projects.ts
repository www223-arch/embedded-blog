...
export const projectItems: ProjectItem[] = loadMarkdownEntries(projectModules)
  ...
  .map((entry) => {
    ...
    return projectSchema.parse({
      ...
      backgroundImage: valueAsString(data.backgroundImage),
      backgroundPosition: valueAsString(data.backgroundPosition),
      backgroundTone: valueAsString(data.backgroundTone),
